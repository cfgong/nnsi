...
Template.survey3.events({
	'submit form': function(event){
		...
		if($(whoareyou).val()==a && $(whoisyourorg).val() == b){
			Origin.update({_id: userId}, {$set:{whoareyou: whoareyou.val(),whoisyourorg: whoisyourorg.val()}});
			Router.go('/survey4');
		}else{
			Router.go('/deadend');
		}
...
Template.survey4.events({
	'submit form': function(event, template){
		...
		Origin.update({_id: userId}, {$set:{primaryAddress: primaryAddress,operatingCountries: array}});
		Router.go('/survey5');
	},
});
...
Template.survey5.events({
	'submit form': function(event){
		...
		Origin.update({_id: userId}, {$set:{budget: budget, revenue: revenue, assets: assets} });
		Router.go('/survey6');
	}
});
...
Template.survey6.events({
	'submit form': function(event){
		...
		Origin.update({_id: userId}, {$set: {organizational_classification: organization_classification} });
		Router.go('/survey7');
	}
});
...
Template.survey7.events({
	'submit form': function(event){
		...
		Origin.update({_id: userId}, {$set: {financial_management: avg} });
		Router.go('/survey8');
	}
});
...
Template.survey8.events({
	'submit form': function(event){
		...
		Origin.update({_id: userId}, {$set: {financial_management2: avg} });
...
Template.survey9.events({
	'submit form': function(event){
		...
		var scoreArray = [q1, q2, q3, q4, q5, q6, q7];
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {adaptive_capacity: avg} });
...
Template.survey10.events({
	'submit form': function(event){
		...
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {strategic_planning: avg} });
...
Template.survey11.events({
	'submit form': function(event){
		...
		var scoreArray = [q1, q2, q3, q4, q5, q6, q7];
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {external_communication: avg} });
...
Template.survey12.events({
	'submit form': function(event){
		...
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {board_leadership: avg} });
...
Template.survey13.events({
	'submit form': function(event){
		...
		var scoreArray = [q1, q2];
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {board_leadership2: avg} });
...
Template.survey14.events({
	'submit form': function(event){
		...
		var scoreArray = [q1, q2, q3, q4, q5];
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {operational_capacity: avg} });
...
Template.survey15.events({
	'submit form': function(event){
		...
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {mission_orientation: avg} });
...
Template.survey16.events({
	'submit form': function(event){
		...
		var scoreArray = [q1, q2, q3, q4];
		var avg = calculateScores(scoreArray);
		Origin.update({_id: userId}, {$set: {staff_management: avg} });
